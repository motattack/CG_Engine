...
#include <CG_Engine_Core/algo/list.h>
...
namespace Octree {
    ...
    */
    ...
    void calculateBounds(BoundingRegion &out, Octant octant, BoundingRegion parentRegion) {
        ...
        Vec3 center = parentRegion.calculateCenter();
        if (octant == Octant::O1) {
            out = BoundingRegion(center, parentRegion.max);
        } else if (octant == Octant::O2) {
            out = BoundingRegion(Vec3(parentRegion.min.x, center.y, center.z),
                                 Vec3(center.x, parentRegion.max.y, parentRegion.max.z));
        } else if (octant == Octant::O3) {
            out = BoundingRegion(Vec3(parentRegion.min.x, parentRegion.min.y, center.z),
                                 Vec3(center.x, center.y, parentRegion.max.z));
        } else if (octant == Octant::O4) {
            out = BoundingRegion(Vec3(center.x, parentRegion.min.y, center.z),
                                 Vec3(parentRegion.max.x, center.y, parentRegion.max.z));
        } else if (octant == Octant::O5) {
            out = BoundingRegion(Vec3(center.x, center.y, parentRegion.min.z),
                                 Vec3(parentRegion.max.x, parentRegion.max.y, center.z));
        } else if (octant == Octant::O6) {
            out = BoundingRegion(Vec3(parentRegion.min.x, center.y, parentRegion.min.z),
                                 Vec3(center.x, parentRegion.max.y, center.z));
        } else if (octant == Octant::O7) {
            out = BoundingRegion(parentRegion.min, center);
        } else if (octant == Octant::O8) {
            out = BoundingRegion(Vec3(center.x, parentRegion.min.y, parentRegion.min.z),
                                 Vec3(parentRegion.max.x, center.y, center.z));
        }
    };
    ...
    */
    ...
    public:
        ...
        */
        ...
        // default
        node() : region(BoundTypes::AABB) {};

        // initialize with bounds (no objects yet)
        node(BoundingRegion bounds) : region(bounds) {};

        // initialize with bounds and list of objects
        node(BoundingRegion bounds, std::vector<BoundingRegion> objectList) : region(bounds) {
            ...
            for (BoundingRegion br: objects) {
                if (br.intersectsWith(obj)) {
                    if (br.instance->instanceId != obj.instance->instanceId) {
                        ...
                        std::cout << "Instance " << br.instance->instanceId << "(" << br.instance->modelId
                                  << ") collides with " << obj.instance->instanceId << "(" << obj.instance->modelId
                                  << ")" << std::endl;
